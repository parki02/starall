[![Build Status](https://travis-ci.org/starall/starall-core.svg?branch=auto)](https://travis-ci.org/starall/starall-core)


# starall-core

Starall-core is a replicated state machine that maintains a local copy of a cryptographic ledger and processes transactions against it, in consensus with a set of peers.
It implements the [Starall Consensus Protocol](https://github.com/starall/starall-core/blob/master/src/scp/readme.md), a _federated_ consensus protocol.
It is written in C++11 and runs on Linux, OSX and Windows.
Learn more by reading the [overview document](https://github.com/starall/starall-core/blob/master/docs/readme.md).

# Documentation

Documentation of the code's layout and abstractions, as well as for the
functionality available, can be found in
[`./docs`](https://github.com/starall/starall-core/tree/master/docs).
...
run tests with:
  `src/starall-core --test`

run one test with:
  `src/starall-core --test  testName`

run one test category with:
  `src/starall-core --test '[categoryName]'`
...
supported test options can be seen with
  `src/starall-core --test --help`

display tests timing information:
  `src/starall-core --test -d yes '[categoryName]'`

xml test output (includes nested section information):
  `src/starall-core --test -r xml '[categoryName]'`
...
Then, running
* `starall-core --test [stress]` will run all the stress tests,
* `starall-core --test [foo-stress]` will run the stress tests for subsystem foo alone, and
* neither `starall-core --test` nor `starall-core --test [foo]` will run stress tests.
