...
starall-core can be controlled via the following commands.

## Command line options
* **--?** or **--help**: Print the available command line options and then exit..
* **--c** Send an [HTTP command](#http-commands) to an already running local instance of starall-core and then exit. For example: 

`$ starall-core -c info`

* **--conf FILE**: Specify a config file to use. You can use '-' and provide the config file via STDIN. *default 'starall-core.cfg'*
...
`$ starall-core --convertid SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
the database. The next time starall-core is run, starall-core will start 
emitting SCP messages based on its last known ledger. Without this flag starall-core waits to hear
...
* **--newhist ARCH**:  Initialize the named history archive ARCH. ARCH should be one of the history archives you have specified in the starall-core.cfg. This will write a `.well-known/starall-history.json` file in the archive root.
...
* **--sec2pub**:  Reads a secret key on standard input and outputs the
  corresponding public key.  Both keys are in Starall's standard
  base-32 ASCII format.
* **--netid STRING**:  The --signtxn option requires a particular
  network to sign for.  For example, the production starall network is
  "`Public Global Starall Network ; September 2015`" while the test
...
* **--test**: Run all the unit tests.
  ...
  * For example this will run just the tests tagged with `[tx]` using protocol
  ...
  `starall-core --test -a --version 9 --version 10 "[tx]"`
* **--version**: Print version info and then exit.


## HTTP Commands
By default starall-core listens for connections from localhost on port 11626. 
You can send commands to starall-core via a web browser, curl, or using the --c 
...
* **setcursor**
 `/setcursor?id=ID&cursor=N`<br>
  ...
  Cursors are used by dependent services to tell starall-core which data can be safely deleted by the instance.
