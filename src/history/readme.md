...
The `get` and `put` commands define subprocesses that `starall-core` will
...
called `s3starall` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3starall]
get="curl http://history.starall.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.starall.org/{1}"
~~~~

In this example, `starall-core` will use the `curl` command to fetch history and
