# What is a ledger?
A ledger represents the state of the Starall universe at a given point in time.
...
[`src/xdr/Starall-ledger.x`](../xdr/Starall-ledger.x)
...
During consensus, nodes work together to decide on the value of StarallValue.
StarallValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'StarallValue' should be able to transition to the same new ledger.
...
[`src/xdr/Starall-ledger-entries.x`](../xdr/Starall-ledger-entries.x)

### AccountEntry
This entry represents an account. In Starall, everything is centered around
...
Offers are a way to automate simple trading inside the Starall network.
