...
class Floodgate
{
    class FloodRecord
    {
      public:
        ...
        uint32_t mLedgerSeq;
        StarallMessage mMessage;
        std::set<Peer::pointer> mPeersTold;

        FloodRecord(StarallMessage const& msg, uint32_t ledger,
                    Peer::pointer peer);
    };
  ...
  public:
    ...
    bool addRecord(StarallMessage const& msg, Peer::pointer fromPeer);

    void broadcast(StarallMessage const& msg, bool force);
