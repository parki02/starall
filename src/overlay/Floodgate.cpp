...
namespace starall
{
Floodgate::FloodRecord::FloodRecord(StarallMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(StarallMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(StarallMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
