...
static std::string
msgSummary(StarallMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(StarallMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(StarallMessage const& starallMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(starallMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (starallMsg.type() != HELLO) &&
        (starallMsg.type() != AUTH) && (starallMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << starallMsg.type() << " before completed handshake";
        mDropInRecvMessageUnauthMeter.Mark();
        drop();
        return;
    }

    assert(isAuthenticated() || starallMsg.type() == HELLO ||
           starallMsg.type() == AUTH || starallMsg.type() == ERROR_MSG);

    switch (starallMsg.type())
    {
    case ERROR_MSG:
    {
        auto t = mRecvErrorTimer.TimeScope();
        recvError(starallMsg);
    }
    break;
    ...
    case HELLO:
    {
        ...
        this->recvHello(starallMsg.hello());
    }
    break;
    ...
    case GET_SCP_QUORUMSET:
    {
        ...
        recvGetSCPQuorumSet(starallMsg);
    }
    break;
...
void
Peer::recvDontHave(StarallMessage const& msg)
{
...
void
Peer::recvGetTxSet(StarallMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(StarallMessage const& msg)
{
...
void
Peer::recvTransaction(StarallMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(StarallMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(StarallMessage const& msg)
{
...
void
Peer::recvSCPMessage(StarallMessage const& msg)
{
...
void
Peer::recvGetSCPState(StarallMessage const& msg)
{
...
void
Peer::recvError(StarallMessage const& msg)
{
...
void
Peer::recvAuth(StarallMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(StarallMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(StarallMessage const& msg)
{
