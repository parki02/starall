// Copyright 2015 Starall Development Foundation and contributors. Licensed
 ...
 *     random StarallMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     starall-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(StarallMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, StarallMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing starall-core " << STARALL_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<StarallMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            StarallMessage m(gen(10));
            out.writeOne(m);
