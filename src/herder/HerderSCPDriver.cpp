...
void
HerderSCPDriver::restoreSCPState(uint64_t index, StarallValue const& value)
{
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    StarallValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<StarallValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        StarallValue& sv = candidateValues.back();
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid StarallValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized StarallValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, StarallValue const& value,
                          TxSetFramePtr proposedSet,
                          StarallValue const& previousValue)
{
