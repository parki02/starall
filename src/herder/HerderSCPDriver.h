...
class HerderSCPDriver : public SCPDriver
{
  public:
    struct ConsensusData
    {
        ...
        StarallValue mConsensusValue;
        ConsensusData(uint64_t index, StarallValue const& b)
    ...
    void restoreSCPState(uint64_t index, StarallValue const& value);
    ...
    void nominate(uint64_t slotIndex, StarallValue const& value,
                  TxSetFramePtr proposedSet, StarallValue const& previousValue);
  ...
  private:
    ...
    validateValueHelper(uint64_t slotIndex, StarallValue const& sv) const;
